...
2. **Queries should be projections** — Use `.Select()` to project into DTOs instead of loading full entities. This avoids over-fetching and N+1 issues.
3. **Migrations are code** — Treat them like any other source code. Review them, test them, never auto-apply in production.
